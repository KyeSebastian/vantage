...
# Attack 1 - url with port should strip correctly

class TestPortInURL:
    """Port numbers in URLs must be stripped so the hostname resolves correctly."""

    def test_url_with_port_resolves_correctly(self):
        t = Target.from_string("https://scanme.nmap.org:8080/path")
...
# Attack 2 - empty/whitespace input must fail clearly
...
class TestInvalidIPFormats:
    ...
    def test_out_of_range_octet_is_rejected(self):
        # 999.999.999.999 looks like an IP but ipaddress rejects it,
    ...
    def test_incomplete_ip_treated_as_hostname_or_rejected(self):
        ...
        with pytest.raises((ValueError, OSError)):
            ...
            assert result.is_ip is False  # acceptable if treated as domain


# Gap 2 - is_ip flag must be correct for every input type

class TestIsIPFlag:
    """is_ip must be True for IPs and False for domains, it controls which modules run."""
